...
- ✅ **Missions Partner Program** - Full end-to-end missions support system: registration form at `/missions/register`, public directory at `/missions` (filterable by type), individual profile pages at `/missions/:slug` with Give/Share/Pray CTAs, admin Missions tab for approving/rejecting submissions, and transparent 90/10 giving model (voluntary group commitment, not system-enforced). URL security hardening enforces http/https-only for all user-submitted links. Nav header, footer, help center, features list, and CTA all updated to include Missions Partners.
- ✅ **All App Screens Mobile Responsive** - Admin tabs scrollable on mobile (7-tab overflow fixed), Home buttons added to all three missions pages, "Missions Partners" added to both desktop and mobile nav header, how-it-works updated to 4-step grid (sm:grid-cols-2 lg:grid-cols-4).
- ✅ **Bible Study Script Generator** - Replaced the "Bible Lookup" nav item with a dedicated Bible Study Generator page at `/bible`. Supports 8 study types (Men's, Women's, Business/Marketplace, Sunday School, Youth Group, Couples, Senior Adults, General). Leaders can upload a sermon script, enter their group name and optional topic, then generate a complete tailored study guide. Removed Men's/Women's Bible Study buttons from the chat interface (moved here).
- ✅ **Bible Games Difficulty Levels** - Added Easy/Medium/Hard difficulty settings and a "New" refresh button to all 6 visual Bible games. Each game adjusts its challenge level based on the selected difficulty.
...
## Previous Updates (January-February 2025)
...
- ✅ **Spell Check Optimization for File Uploads** - Removed spell check functionality for attached file content to prevent processing large documents and focus spell check only on user-typed input (February 2025)
